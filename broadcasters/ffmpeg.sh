...
#x264
srtpKey=`echo -ne "I_am_srtp_key_30_bytes_1234567"|base64`
echo ${srtpKey}
ffmpeg \
	 \
	-re \
	...
	-srtp_out_suite AES_CM_128_HMAC_SHA1_80 -srtp_out_params ${srtpKey} \
	-f tee \
	"[select=v:f=rtp:ssrc=${VIDEO_SSRC}:payload_type=${VIDEO_PT}]srtp://${videoTransportIp}:${videoTransportPort}?rtcpport=${videoTransportRtcpPort}"
